...
// Transfer-price adjustments (pending charges or credits from room moves)
$transfer_adjustments = [];
try {
    $adj_stmt = $pdo->prepare(
        "SELECT ta.id, ta.amount, ta.direction, ta.old_price, ta.new_price,
                ta.notes, ta.status, ta.created_at,
    ...
    $adj_stmt->execute([$uid]);
    $transfer_adjustments = $adj_stmt->fetchAll();
} catch (Exception $e) { /* table may not exist yet */ }
$pending_adj_charge = 0;
$pending_adj_credit = 0;
foreach ($transfer_adjustments as $a) {
    if ($a['status'] === 'pending') {
        if ($a['direction'] === 'charge') $pending_adj_charge += (float)$a['amount'];
        if ($a['direction'] === 'credit') $pending_adj_credit += (float)$a['amount'];
    }
}
...
<head>
    ...
    <style>
        ...
        html[data-theme="dark"] .bill-card { background: var(--panel); }

        /* Transfer adjustments */
        .transfer-adj-card .bill-card-body { padding: 0.85rem 1.25rem 1.1rem; display: flex; flex-direction: column; gap: 0.6rem; }
        .adj-item {
        ...
        .adj-item.adj-charge { background: var(--panel-alt); border-color: var(--border); }
        .adj-item.adj-credit { background: var(--panel-alt); border-color: var(--border); }
        .adj-icon {
        ...
        .adj-icon svg { width: 16px; height: 16px; stroke: currentColor; fill: none; stroke-width: 2.2; }
        .adj-charge .adj-icon { background: var(--primary-soft); color: var(--primary-strong); }
        .adj-credit .adj-icon { background: var(--primary-soft); color: var(--primary-strong); }
        html[data-theme="dark"] .adj-charge .adj-icon { color: var(--primary-light); }
        html[data-theme="dark"] .adj-credit .adj-icon { color: var(--primary-light); }
        .adj-body { min-width: 0; flex: 1; }
        .adj-title { font-size: 0.92rem; font-weight: 800; color: var(--text); letter-spacing: -0.01em; display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap; }
        .adj-meta { font-size: 0.78rem; color: var(--muted); margin-top: 3px; line-height: 1.4; }
        .adj-status { font-size: 0.66rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.08em; padding: 0.18rem 0.55rem; border-radius: 999px; }
        .adj-status.pending { background: rgba(245,158,11,0.16); color: #b45309; }
        .adj-status.settled { background: rgba(100,116,139,0.16); color: #475569; }
        html[data-theme="dark"] .adj-status.pending { background: rgba(251,191,36,0.18); color: #fbbf24; }
        html[data-theme="dark"] .adj-status.settled { background: rgba(148,163,184,0.2); color: #cbd5e1; }
        .adj-summary { display: flex; flex-direction: column; gap: 0.4rem; margin-top: 0.4rem; }
        .adj-sum-pill { font-size: 0.82rem; padding: 0.5rem 0.75rem; border-radius: 8px; line-height: 1.4; }
        .adj-sum-pill.charge  { background: var(--panel-alt); color: var(--text); border: 1px solid var(--border); }
        .adj-sum-pill.credit  { background: var(--panel-alt); color: var(--text); border: 1px solid var(--border); }
        html[data-theme="dark"] .adj-sum-pill.charge { background: var(--panel-alt); color: var(--text); border-color: var(--border); }
        html[data-theme="dark"] .adj-sum-pill.credit { background: var(--panel-alt); color: var(--text); border-color: var(--border); }
...
<div class="app-shell">
    ...
    <main class="shell-main">
        ...
        <!-- Transfer adjustments -->
        <?php if (!empty($transfer_adjustments)): ?>
        <div class="bill-card transfer-adj-card">
            <div class="bill-card-head">
                <span class="bill-card-title">Room Transfer Adjustments</span>
            </div>
            <div class="bill-card-body">
                <?php foreach ($transfer_adjustments as $a):
                    $is_charge  = $a['direction'] === 'charge';
                    $is_pending = $a['status']    === 'pending';
                ?>
                <div class="adj-item adj-<?= $is_charge ? 'charge' : 'credit' ?>">
                    <div class="adj-icon">
                        <?php if ($is_charge): ?>
                    ...
                    <div class="adj-body">
                        <div class="adj-title">
                            <?php if ($is_charge): ?>You owe NPR <?= number_format((float)$a['amount'], 0) ?><?php else: ?>Refund due: NPR <?= number_format((float)$a['amount'], 0) ?><?php endif; ?>
                            <span class="adj-status <?= $is_pending ? 'pending' : 'settled' ?>"><?= $is_pending ? 'Pending' : 'Settled' ?></span>
                        </div>
                        <div class="adj-meta">
                            Room <?= e($a['from_room_num'] ?? '?') ?> (NPR <?= number_format((float)$a['old_price'], 0) ?>)
                            &rarr;
                            Room <?= e($a['to_room_num'] ?? '?') ?> (NPR <?= number_format((float)$a['new_price'], 0) ?>)
                            &middot;
                ...
                <?php endforeach; ?>
                <?php if ($pending_adj_charge > 0 || $pending_adj_credit > 0): ?>
                <div class="adj-summary">
                    <?php if ($pending_adj_charge > 0): ?>
                        <span class="adj-sum-pill charge">Outstanding charges: <strong>NPR <?= number_format($pending_adj_charge, 0) ?></strong> &mdash; please clear with the warden or owner.</span>
                    <?php endif; ?>
                    <?php if ($pending_adj_credit > 0): ?>
                        <span class="adj-sum-pill credit">Refund owed to you: <strong>NPR <?= number_format($pending_adj_credit, 0) ?></strong> &mdash; the owner will reach out to settle this.</span>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
            </div>
