...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'approve') {
            ...
            // Free the student's old room only if no other student is assigned to it.
            ...
            $pdo->prepare(
                "UPDATE rooms SET status = 'occupied', student_id = ? WHERE id = ?"
...
<head>
    ...
    <style>
        ...
        .transfer-reason { font-size: 0.85rem; color: var(--text); background: var(--panel-alt); border-left: 3px solid var(--primary); border-radius: 6px; padding: 0.65rem 0.9rem; margin-bottom: 1rem; line-height: 1.5; }
