...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'mark_maintenance') {
            ...
            // Also block partial occupancy — students still living here need
            // to be relocated first.
            $occ_stmt = $pdo->prepare("SELECT COUNT(*) FROM users WHERE room_id = ? AND role = 'student'");
            $occ_stmt->execute([$room_id]);
            if ((int)$occ_stmt->fetchColumn() > 0) {
                throw new Exception('Room has students living in it. Relocate them first.');
            }
...
// Pull each room with its full occupant list. We aggregate through
// users.room_id (the canonical owner) — rooms.student_id is a legacy single
// occupant column that doesn't account for multi-bed rooms.
$rooms = $pdo->query(
    "SELECT r.id, r.room_number, r.room_type, r.floor, r.status, r.capacity,
            r.price, r.notes, r.photo,
...
<head>
    ...
    <style>
        ...
        .s-partial     { background:rgba(245,158,11,0.9);  color:#fff; }
...
<div class="container">
    ...
    <?php foreach ($rooms as $r):
        ...
        $occupants = (int)$r['occupant_count'];
        $capacity  = (int)$r['capacity'];

        // Status used for both the badge and the action lock.
        if ($r['status'] === 'maintenance') {
            $badge_cls = 's-maintenance';
            $badge_lbl = 'Maintenance';
        } elseif ($r['status'] === 'occupied' || $occupants >= $capacity) {
            $badge_cls = 's-occupied';
            $badge_lbl = 'Occupied';
        } elseif ($occupants > 0) {
            $badge_cls = 's-partial';
            $badge_lbl = $occupants . '/' . $capacity . ' Filled';
        } else {
            $badge_cls = 's-available';
            $badge_lbl = 'Available';
        }
        $has_occupants = $occupants > 0;
    ?>
        <div class="rm-card status-<?= e($r['status']) ?>">
            <div class="rm-img">
                ...
                <span class="rm-status-badge <?= $badge_cls ?>"><?= e($badge_lbl) ?></span>
            ...
            <div class="rm-body">
                ...
                <div style="font-size:0.78rem;color:var(--muted);"><?= $occupants ?>/<?= $capacity ?> beds occupied</div>
                <?php if ($has_occupants): ?>
                    <div style="display:flex;align-items:center;gap:0.4rem;margin-bottom:0.5rem;font-size:0.78rem;">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:13px;height:13px;flex-shrink:0;color:var(--muted);"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                        <span style="font-weight:600;color:var(--text);"><?= e($r['occupant_names']) ?></span>
                    </div>
                <?php else: ?>
                    <div style="min-height:18px;margin-bottom:0.5rem;" aria-hidden="true"></div>
                <?php endif; ?>
                <div class="rm-actions">
                    ...
                    <?php elseif (!$has_occupants && $r['status'] !== 'occupied'): ?>
                    ...
                    <?php else: ?>
                        <span style="font-size:0.78rem;color:var(--muted);padding:0.35rem 0;">In use — cannot modify</span>
                    <?php endif; ?>
                </div>
            </div>
        </div>
