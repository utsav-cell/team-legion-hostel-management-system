...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['approve_room'])) {
    ...
    // Only flip the room to 'occupied' if it has now reached capacity — partially
    // filled multi-bed rooms must stay 'available' so other students can still book.
    $pdo->prepare(
        "UPDATE rooms r
         SET r.status = 'occupied', r.is_approved = 1
         WHERE r.id = ?
           AND (SELECT COUNT(*) FROM users u WHERE u.room_id = r.id AND u.role = 'student') >= r.capacity"
    )->execute([$rid]);
    $pdo->prepare("UPDATE rooms SET is_approved = 1 WHERE id = ?")->execute([$rid]);
...
try {
    $rstmt = $pdo->query(
        "SELECT u.id, u.name, u.email, u.photo, r.room_number, u.room_status
...
$wardens = [];
try {
    $ws = $pdo->query("SELECT id, name, email, photo FROM users WHERE role = 'warden' ORDER BY name ASC");
    $wardens = $ws->fetchAll();
} catch (Exception $e) {}
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                ...
                                ?>
                                <a href="../auth/profile.php?id=<?= (int)$p['sid'] ?>" style="display:flex;align-items:center;gap:0.75rem;text-decoration:none;color:inherit;" title="View profile">
                ...
                <div class="table-wrap">
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <a href="../auth/profile.php?id=<?= (int)$w['id'] ?>" style="display:flex;align-items:center;gap:0.65rem;text-decoration:none;color:inherit;" title="View profile">
                ...
                <div class="table-wrap" style="max-height:320px;overflow-y:auto;">
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <a href="../auth/profile.php?id=<?= (int)$s['id'] ?>" style="display:flex;align-items:center;gap:0.65rem;text-decoration:none;color:inherit;" title="View profile">
                                        <div class="rp-avatar" style="width:32px;height:32px;font-size:0.72rem;">
