...
// Single source of truth for the currently open confirm
let _hmsCurrentConfirm = null;
...
function hmsConfirm(message, onConfirm, title, onCancel) {
    buildConfirmModal();
    ...
    // Wire cancel/close/backdrop so we can restore form state if user bails
    const cancelBtn = document.getElementById('hms-cm-cancel');
    const closeBtn  = document.getElementById('hms-cm-close');
    const handleCancel = () => {
        overlay.classList.remove('open');
        if (typeof onCancel === 'function') onCancel();
    };
    const cancelFresh = cancelBtn.cloneNode(true);
    cancelBtn.parentNode.replaceChild(cancelFresh, cancelBtn);
    cancelFresh.addEventListener('click', handleCancel);
    const closeFresh = closeBtn.cloneNode(true);
    closeBtn.parentNode.replaceChild(closeFresh, closeBtn);
    closeFresh.addEventListener('click', handleCancel);
    ...
    document.querySelectorAll('form[data-confirm]').forEach((form) => {
        form.addEventListener('submit', (e) => {
            ...
            hmsConfirm(
                ...
                undefined,
                () => {
                    // Cancel: restore button state so nothing feels stuck
